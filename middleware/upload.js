...
// Dynamic folder ke saath upload middleware
const cloudinaryUpload = (folderName = 'general') => {
  const upload = uploadToCloudinary(folderName);
  ...
  return {
    ...
    // Fields upload (different fields ke liye)
    fields: (fields) => {
...
// Response processor - Cloudinary se URL nikalne ke liye
const processCloudinaryResponse = (req, res, next) => {
  if (req.file) {
    req.file.fullUrl = req.file.path; // Cloudinary URL already in path
    req.file.folder = req.file.folder; // Cloudinary folder
...
module.exports = { cloudinaryUpload, processCloudinaryResponse };