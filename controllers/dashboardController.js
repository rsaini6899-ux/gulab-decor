...
const getLowStockProducts = async () => {
  try {
    ...
    for (const product of products) {
      ...
      // Calculate total stock from variations only
      let totalVariationsStock = 0;
      
      // Add all variations stock
      if (product.variations && product.variations.length > 0) {
        product.variations.forEach(variation => {
          const variationStock = variation.stock || 0;
          totalVariationsStock += variationStock; // ✅ Use += to sum all variations
        });
      }
      
      // Check if total variations stock is low or zero
      const isLowStock = totalVariationsStock > 0 && totalVariationsStock <= threshold;
      const isOutOfStock = totalVariationsStock === 0;
      
      if (isLowStock || isOutOfStock) {
        lowStockProducts.push({
          id: product._id,
          name: product.name,
          totalStock: totalVariationsStock,  // ✅ Total variations stock only
          threshold: threshold,
          totalVariations: product.variations?.length || 0,
    ...
    // Return top 5 most critical low stock products
    return lowStockProducts.slice(0, 5);
