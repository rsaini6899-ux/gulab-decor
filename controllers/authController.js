  ...
  try {
    ...
    if (!user) {
      user = await User.create({
        ...
        isActive: true,
        lastLogin: Date.now(),
        hasPassword: false, // Mark that user doesn't have a password set
      });
    } else {
      user.lastLogin = Date.now();
      await user.save();
  ...
  try {
    ...
    if (user) {
      ...
      user = await User.create({
        ...
        resetPasswordExpire: otpExpire,
        isActive: false, // User not fully registered yet
        hasPassword: false, // No password set
      });
    }
...
// ✅ NEW: Email + Password Login with Auto Password Setup
exports.login = async (req, res, next) => {
  try {
    const { email, password } = req.body;
    ...
    // ✅ If user doesn't have a password (registered via Google/OTP)
    if (!hasPassword || user.hasPassword === false) {
      // This is the first time user is trying to login with password
      ...
      return res.status(200).json({
        ...
        isFirstTimeLogin: true // Flag to indicate password was just set
      });
    }
...
exports.changePassword = async (req, res, next) => {
  try {
    ...
    if (!user.hasPassword && !user.password) {
      // User never set a password, just set it
      user.password = newPassword;
      user.hasPassword = true;
      await user.save();
